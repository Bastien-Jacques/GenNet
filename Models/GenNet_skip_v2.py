...
# Ajout de 2 couches hidden dans le décodeur Cd
# Ajout de Weightnormalisation dans chaque couche 
# Ajout de la fonction softplus dans la prédiction du Cd
# Ajout de Tanh dans la prédiction SDF
...
class SDFEncoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim):
        super().__init__()
        self.fc1 = weight_norm(nn.Linear(4, hidden_dim))   # (x, y, z, sdf) --> hidden_dim
        self.fc2 = weight_norm(nn.Linear(hidden_dim, hidden_dim))
        self.fc3 = weight_norm(nn.Linear(hidden_dim, latent_dim))
...
class SDFDecoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim, dropout):
        super().__init__()
        self.fc1 = weight_norm(nn.Linear(latent_dim + 3, hidden_dim)) #première couche fully connected letent_dim + 3 --> hidden_dim
        self.fc2 = weight_norm(nn.Linear(hidden_dim, hidden_dim)) # couche fully-connected hidden_dim --> hidden_dim
        self.fc3 = weight_norm(nn.Linear(hidden_dim, hidden_dim))
        self.fc4 = weight_norm(nn.Linear(hidden_dim + latent_dim + 3, hidden_dim)) # skip connection: on réinjecte le tatent + vecteur position
        self.fc5 = weight_norm(nn.Linear(hidden_dim, hidden_dim)) # couche fully-connected hidden_dim --> hidden_dim
        self.fc6 = weight_norm(nn.Linear(hidden_dim, hidden_dim))

        self.cd_fc1 = weight_norm(nn.Linear(latent_dim, hidden_dim))
        self.cd_fc2 = weight_norm(nn.Linear(hidden_dim, hidden_dim))

        # Double tête
        self.head_sdf = weight_norm(nn.Linear(hidden_dim, 1)) # tête de sortie SDF: hidden_dim --> 1
        self.head_cd = weight_norm(nn.Linear(hidden_dim, 1)) #tête de sortie Cd
...
class AutoencoderSDF(nn.Module):
    ...
    def predict_cd_only(self, z):
        f = F.relu(self.decoder.cd_fc1(z))
        f = F.relu(self.decoder.cd_fc2(f))
        cd = self.decoder.head_cd(self.decoder.dropout_cd(f))
        cd = F.softplus(cd)
        return cd